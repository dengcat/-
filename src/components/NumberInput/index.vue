...
export default {
  ...
  watch: {
    ...
    valueNumber (newValue) {
      if (this.$refs.numeric && this.$refs.numeric.$refs.input !== document.activeElement) {
  ...
  methods: {
     ...
     */
    onBlurHandler (e) {
      const fixedValue = accounting.toFixed(this.amountNumber, this.precision)
      const output = this.outputType.toLowerCase() === 'string' ? fixedValue : Number(fixedValue)
      this.$emit('blur', output, e)
      this.amount = this.format(this.valueNumber)
    },
