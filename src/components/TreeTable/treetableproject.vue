<template>
  <el-table row-key="businessDataGUID" :data="formatData" :row-style="showRow" v-bind="$attrs" v-on:row-click="$emit('row-click',$event)">
    ...
    <el-table-column v-for="(column, index) in columns" v-else :key="column.value" :label="column.text" :width="column.width" :min-width="column.minWidth">
      <template slot-scope="scope">
        ...
        <svg-icon :icon-class="scope.row.businessSimplifyName | iconFilter" style="margin-right:5px;color:#ccc" />
