...
export function treeTransArray(tree, key) {
  return tree.reduce((con, item) => {
    ...
    if (item[key] && item[key].length > 0) {
      item[key].reduce(treeTransArray, con)
    }
    return con
  }, [])
}

 export function arrayTransTree(list, parentId, Id) {
   if (list === undefined) {
     return list
   }
  var tree = list.filter((parent) => {
    ...
    return parent[parentId] === '' || parent[parentId] === null
  })
  return tree
}

export function arrayTransTreearray(list, parentId, Id) {
  if (list === undefined) {
    return list
  }
  var tree = list.filter((parent) => {
...
// 树转换为数组
export function treeToArrayNoParent(data, parent = null, level = null) {
  let tmp = []
  Array.from(data).forEach(function(record) {
    let _level = 1
    if (level !== undefined && level !== null) {
    ...
    tmp.push(record)
    if (record.children && record.children.length > 0) {
      const children = treeToArrayNoParent(record.children, record, _level)
      tmp = tmp.concat(children)
    }
  })
  return tmp
