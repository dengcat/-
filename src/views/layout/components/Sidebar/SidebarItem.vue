<template>
  <div v-if="!item.hidden&&item.children" class="menu-wrapper">
    ...
    <template v-if="hasOneShowingChild(item.children) && !onlyOneChild.children&&!item.alwaysShow">
      <a :href="onlyOneChild.path" target="_blank" @click="clickLink(onlyOneChild.path,onlyOneChild.meta,$event)">
    ...
    <el-submenu v-else :index="item.name||item.path">
      ...
      <template v-for="child in item.children" v-if="!child.hidden">
        ...
        <a v-else :href="child.path" :key="child.name" target="_blank" @click="clickLink(child.path,child.meta,$event)">
  ...
  methods: {
    ...
    clickLink(routePath,meta, e) {
      if (!this.isExternalLink(routePath)) {
        ...
        if(meta.token){
          routePath = `${routePath}?token=${token}`
        }
        // const path = this.resolvePath(routePath)
