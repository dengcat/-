  ...
  <div v-loading ='loading'>
    <el-form ref="form" :model="form" label-position="left" label-width="100px">
      ...
      <el-row>
        ...
        <el-col :span="15">
          <span v-for="item in showTags" :key="item.id"><el-tag  @click="handleCheckTag(item)" size="small" style="margin-right:10px;cursor:pointer"   type="info">{{item.name}}</el-tag></span>
          <span style="margin:0 auto;" v-if="showTags.length == 0">无可选数据</span>
        </el-col>
        <el-col style="height:100%" :span="4">
          <div class="animate item item2 js_keyboard" @click="handleCheckAri({name:'+',key: 0})" data-key="0"><span class="animate"></span></div>
          <div class="animate item item3 js_keyboard" @click="handleCheckAri({name:'-',key: 0})" data-key="0"><span class="animate"></span></div>
          <div class="animate item item4 js_keyboard" @click="handleCheckAri({name:'*',key: 0})" data-key="0"><span class="animate"></span></div>
          <div class="animate item item5 js_keyboard" @click="handleCheckAri({name:'÷',key: 0})" data-key="0"><span class="animate"></span></div>
          <div class="animate item item6 js_keyboard" @click="handleCheckAri({name:'(',key: 4,brackets:'left'})" data-key="0"><span class="animate"></span></div>
          <div class="animate item item6 js_keyboard" @click="handleCheckAri({name:')',key: 4,brackets:'right'})" data-key="0"><span class="animate"></span></div>
          <div class="animate item item7 js_keyboard" @click="handleCheckAri({name:'%',key: 5})" data-key="0"><span class="animate"></span></div>
          <!-- <div @click="handleCheckTag({name:',',key: 0})" class="animate item char1 js_keyboard" data-key="0"><span class="animate"></span></div>
          <div @click="handleCheckTag({name:';',key: 0})" class="animate item char2 js_keyboard" data-key="0"><span class="animate"></span></div>
          <div @click="handleCheckTag({name:'-',key: 0})" class="animate item char3 js_keyboard" data-key="0"><span class="animate"></span></div>
          <div @click="handleCheckTag({name:'#',key: 0})" class="animate item char4 js_keyboard" data-key="0"><span class="animate"></span></div>
          <div @click="handleCheckTag({name:'|',key: 0})" class="animate item char5 js_keyboard" data-key="0"><span class="animate"></span></div>
          <div @click="handleCheckTag({name:'·',key: 0})" class="animate item char6 js_keyboard" data-key="0"><span class="animate"></span></div>
  ...
  methods: {
    ...
    getFormulaCondition: function() {
      ...
      if (self.formulaCondition.length) {
        ...
        const tempObj = { formula: data.formulaCondition };
        data.formulaCondition = JSON.stringify(tempObj);
    ...
    validate() {
      ...
      if (firstFormula.hasOwnProperty("key") && firstFormula.key != 4) {
      ...
      if (lastFormula.hasOwnProperty("key") && lastFormula.key != 4) {
        //结尾公式条件如果是公式且公式不是括弧，验证不通过
        return false;
      }
        ...
        if (index >= 1) {
          ...
          if (item.hasOwnProperty("key")) {
            ...
            if (item.key == 4) {
              ...
              //括弧外的其他计算条件
              return prevItem.hasOwnProperty("key") && prevItem.key != 4; //不能连续出现公式
            }
          } else {
            //除开计算条件外公式以外
            if (prevItem.hasOwnProperty("key")) {
              ...
              return !prevItem.hasOwnProperty("key"); //指标前不能是指标
            }
          }
        }
      });
