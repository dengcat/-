  ...
  <div v-loading="loading" class="app-container">
    <el-row :gutter="12">
      ...
      <el-col style="text-align:right" :span="18">  
        ...
        <el-button @click="handleExportProject()" >导出excel</el-button>
            ...
            <tree-table-project style="width: 100%; margin-top:10px" :data="projectData" :columns="columns" :expand-all="expandAll" border>
              ...
              <el-table-column label="操作" width="200">
                 <template slot-scope="scope">
                  ...
                  :to="{name: 'MdmProjectObject', params: { projectId:id,projectName:project.name,templateGUID: templateGUID, parentId: scope.row.parentGUID?scope.row.parentGUID:'00000000-0000-0000-0000-000000000000', type: scope.row.businessSimplifyName, id: scope.row.businessDataGUID }}">
                    <el-button v-permission="[{functionCode: '1013', actionCodes: '03'}]" type="text">编辑</el-button>
                  </router-link>
                  <router-link v-if="(project.approveState==2 || project.approveState==5) || ((scope.row.approveState!==2 && scope.row.approveState!==5) && checkPermission([{functionCode: '1013', actionCodes: '00'}]) && !checkPermission([{functionCode: '1013', actionCodes: '03'}]))" 
                  :to="{name: 'MdmProjectObject', params: { projectId:id,projectName:project.name,templateGUID: templateGUID, parentId: scope.row.parentGUID?scope.row.parentGUID:'00000000-0000-0000-0000-000000000000', type: scope.row.businessSimplifyName, id: scope.row.businessDataGUID }}">
                    <el-button type="text" >查看</el-button>
                  </router-link>
                  <router-link v-permission="[{functionCode: '1013', actionCodes: '03'}]" v-if="(project.approveState==0 || project.approveState==1) && scope.row.modifyState!==3" 
                  :to="{name: 'MdmProjectObject', params: { projectId:id,projectName:project.name,templateGUID: templateGUID, parentId: scope.row.businessDataGUID, type: scope.row.childBusinessSimplifyName }}">
...
export default {
  ...
  methods: {
    ...
    initPage(){
      getProjectDetailBusinessData(this.id).then(
        response => {
          ...
          this.loading =false
          this.$route.meta.title =projectTreeInfo[0].name
    ...
    handleUnDeleteBusinessData(row){
      this.loading = true
    ...
    handleExportProject(){
      window.location.href=`${this.BASE_API}api/services/app/projectManage/ExportExcel?businessDataGuid=${this.id}`;
    }
  }
};
</script>
