<template>
  ...
  >
    ...
    >
      ...
      ></i>
      <span style="margin-left:10px;">供货计划-楼栋明细表</span>
      <span style="float:right;height:21px;padding:0px">供货年限:
        <el-select
          size="mini"
...
export default {
  ...
  mixins: [mixin_commom],
  data() {
    const year = this.setting && this.setting.years ? this.setting.years : 4;
    const wd =
      this.setting && this.setting.dimension ? this.setting.dimension : "季度";
    return {
      price: undefined,
      year,
      wd,
  ...
  computed: {
    showData: function() {
      const data = this.mpiSupplyBuildInfo;
      if (data) {
    ...
    currentYear: function(){
      if(this.mpiVersionInfo && this.mpiVersionInfo.year){
  ...
  methods: {
    ...
    handleSave(type) {
      ...
      // 更新父级
      // 父节点
      const parentNode = this.mpiSupplyBuildInfo.find(item => {
    ...
    calcTotal(row) {
      ...
      row.datas.forEach(item => {
        ...
        if (
          ...
          totalAvgPrice = numeral(totalPrice).multiply(10000)
            .divide(totalArea)
            .value();
        }
      });
