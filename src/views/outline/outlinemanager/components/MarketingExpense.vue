<template>
  <el-card class="editCard" v-loading="pageLoading">
    ...
    >
      ...
      ></i>
      <span style="margin-left:10px;">营销费用（含税）</span>
...
export default {
  ...
  computed: {
    showData() {
      const data = this.mpiMarketingCostInfo;
      if (data) {
  ...
  methods: {
    ...
    calcManager(){
      // 合计
      let lsssfy =0 // 临时设施
      let zjf =0 // 无形资产及其他资产购置支出
      let xcsfdwzxgc = 0 // 现场示范单位装修工程
      let zctx = 0  // 资产摊销
      let cost = 0 //一级费用科目
      this.arrData.forEach((item,index)=>{
        ...
        if(item.costName==='临时设施费用' && item.sumCost){
          lsssfy = lsssfy + item.sumCost
        }
        if(item.sumCost && item.costName==='折旧费'){
          zjf = zjf + item.sumCost
        }
        if(item.sumCost && item.costName==='现场示范单位装修工程'){
          xcsfdwzxgc = xcsfdwzxgc + item.sumCost
        }
        if(item.sumCost && item.costName==='资产摊销'){
          zctx = zctx + item.sumCost
        }
      })
      this.arrData[0].sumCost = cost - lsssfy - xcsfdwzxgc
      ...
      this.arrData[0].datas.forEach((item,index)=>{
        ...
        this.arrData.forEach((dataitem,dataindex)=>{
          ...
          if(item.costName==='临时设施费用' && dataitem.datas[index].costValue){
            lsssfy = lsssfy + dataitem.datas[index].costValue
          }
          if(dataitem.datas[index].costValue && item.costName==='折旧费'){
            zjf = zjf + dataitem.datas[index].costValue
          }
          if(dataitem.datas[index].costValue && item.costName==='现场示范单位装修工程'){
            xcsfdwzxgc = xcsfdwzxgc + dataitem.datas[index].costValue
          }
          if(dataitem.datas[index].costValue && item.costName==='资产摊销'){
            zctx = zctx + dataitem.datas[index].costValue
          }
        })
        this.arrData[0].datas[index].costValue = cost - lsssfy - xcsfdwzxgc
