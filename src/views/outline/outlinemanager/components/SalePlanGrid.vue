<template>
  ...
  >
    ...
    >
      ...
      ></i>
      <span style="margin-left:10px;">销售计划</span>
    ...
    <template>
      ...
      >
        ...
        <!-- 供货面积 -->
        <el-table-column :label="`合同销售面积（平米）`" :key="`合同销售面积_${year}_${wd}`">
          <el-table-column
            header-align="center"           align="right"
            :label="`${currentYear}年前已销售`"
            width="120"
            :prop="`contractArea_${currentYear}年前`"
            :show-overflow-tooltip="true"
          >
            <template slot-scope="scope">
              {{scope.row[`contractArea_${currentYear}年前`] | numberFormat}}
            </template>
          </el-table-column>
          <el-table-column
            v-for="yearindex in year"
            :key="`contractArea_${currentYear+yearindex-1}_${year}_${wd}`"
            :label="`${currentYear+yearindex-1}年`"
            header-align="center"           align="right"
            :show-overflow-tooltip="true"
            :prop="`contractArea_${currentYear+yearindex-1}年`"
          >
           <template v-if="wdArr.length === 0" slot-scope="scope">
              {{scope.row[`contractArea_${currentYear+yearindex-1}年`] | numberFormat}}
            </template>
            <el-table-column
              v-for="wditem in wdArr"
              :key="`contractArea_${currentYear+yearindex-1}年_${wditem}_${year}_${wd}`"
              :label="wditem"
              header-align="center"           align="right"
              :width="120"
              :show-overflow-tooltip="true"
              :prop="`contractArea_${currentYear+yearindex-1}年_${wditem}`"
            >
              <template slot-scope="scope">
                {{scope.row[`contractArea_${currentYear+yearindex-1}年_${wditem}`] | numberFormat}}
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column
            ...
            :width="120"
            :show-overflow-tooltip="true"
            :prop="`contractArea_${currentYear+year-1}年后`"
          >
            <template slot-scope="scope">
              {{scope.row[`contractArea_${currentYear+year-1}年后`] | numberFormat}}
            </template>
          </el-table-column>
        </el-table-column>
        <!-- 供货金额 -->
        <el-table-column :label="`合同销售金额（万元）`" :key="`合同销售金额金额_${year}_${wd}`">
          <el-table-column
            :label="`${currentYear}年前已销售`"
            :width="120"
            header-align="center"           align="right"
    ...
    >
      ...
      >
        ...
        <el-table-column label="合同销售面积(平米)">
          <template slot-scope="scope">
            <edit-grid-item
              :value="scope.row.contractArea"
              type='number'
              @input="val => handleInput(val, scope.row, 'contractArea')"
              :isEdit="(currEditRow.children && currEditRow.children.length > 0) ? false : true"
            ></edit-grid-item>
          </template>

        </el-table-column>
        <el-table-column label="合同销售金额(万元)">
          <template slot-scope="scope">
            <edit-grid-item
              :value="scope.row.contractPrice"
              type='number'
              @input="val => handleInput(val, scope.row, 'contractPrice')"
              :isEdit="(currEditRow.children && currEditRow.children.length > 0) ? false : true"
            ></edit-grid-item>
          </template>
        </el-table-column>
...
export default {
  ...
  computed: {
    showData: function() {
      const data = this.mpiSalePlanInfo;
      if (data) {
    ...
    arrData: function() {
      return treeToArrayNoParent(this.mpiSalePlanInfo);
    },
    arrMpiSaleCarryOverInfo: function() {
      return treeToArrayNoParent(this.mpiSaleCarryOverInfo);
    },
    arrMpiReturnPlanInfo: function() {
  ...
  methods: {
    ...
    calcTotal(row){
      ...
      row.datas.forEach(item=>{
        ...
        if(numeral.isNumeral(numeral(totalPrice)) && numeral.isNumeral(numeral(totalArea)) && totalArea!==0){
          totalAvgPrice = numeral(totalPrice).multiply(10000).divide(totalArea).format('0,0.00')
        }
      })
    ...
    handleSave(type) {
      ...
      this.currEditRow.datas.forEach(
        (item,$index) => {
          this.$set(item,'contractArea',this.editRowInfo.datas[$index].contractArea)
          ...
          this.$set(item, "cerSourceDate", this.editRowInfo.cerSourceDate);
        }
      )
      // 更新父级
      // 父节点
      const parentNode = this.mpiSalePlanInfo.find(item => {
      ...
      if (parentNode && parentNode.children) {
        ...
        parentNode.datas.forEach(parentItem => {
          ...
          parentNode.children.forEach(item => {
            item.datas.forEach(datasItem =>{
              if(datasItem.year === parentItem.year && datasItem.quarterOrMonth === parentItem.quarterOrMonth){
                if (datasItem['contractArea']) {
                  sumContractArea = sumContractArea + datasItem['contractArea'];
                }
          ...
          this.$set(parentItem,'contractArea',sumContractArea)
    ...
    updateRowNode(row) {
      ...
      row.datas.forEach(datasItem => {
        ...
        const key_contractArea = datasItem.quarterOrMonth
          ? `contractArea_${datasItem.year}_${datasItem.quarterOrMonth}`
          : `contractArea_${datasItem.year}`;
        dynamicsObj[key_contractPrice] = datasItem.contractPrice;
        dynamicsObj[key_contractArea] = datasItem.contractArea;
      });
    ...
    updateReturnAndCarryover(){
      this.arrData.forEach(item=>{
        const itemSaleCarryOver = this.arrMpiSaleCarryOverInfo.find(element=>{
          return element.sectionName===item.sectionName && element.productType===item.productType && element.productName===item.productName
        })
        const itemReturnPlan = this.arrMpiReturnPlanInfo.find(element=>{
          return element.sectionName===item.sectionName && element.productType===item.productType && element.productName===item.productName
        })
        itemSaleCarryOver.totalPrice = item.totalPrice
        itemSaleCarryOver.totalArea = item.totalArea
        itemSaleCarryOver.totalAvgPrice = item.totalAvgPrice
        itemSaleCarryOver.datas.forEach(datasitem=>{
          datasitem.totalPrice = item.totalPrice
          datasitem.totalArea = item.totalArea
          datasitem.totalAvgPrice = item.totalAvgPrice
        })
        itemReturnPlan.totalPrice = item.totalPrice
        itemReturnPlan.totalArea = item.totalArea
        itemReturnPlan.totalAvgPrice = item.totalAvgPrice
        itemReturnPlan.datas.forEach(datasitem=>{
          datasitem.totalPrice = item.totalPrice
          datasitem.totalArea = item.totalArea
          datasitem.totalAvgPrice = item.totalAvgPrice
        })
