<template>
  <el-card class="editCard">
    ...
    >
      ...
      ></i>
      <span style="margin-left:10px;">股东层面现金流计算表</span>
    </div>
    <template>
      <el-table
        :data="showData"
      ...
      >
        ...
        <template slot-scope="scope">
          {{scope.row.sumPrice | numberFormat}}
        </template>
        </el-table-column>
        <el-table-column
          label="小计"
    ...
    >
      ...
      >
        ...
        <el-table-column label="现金流" header-align="center"    :show-overflow-tooltip="true"       align="right">
          <template slot-scope="scope">
            <edit-grid-item
              :value="scope.row.price"
              type='number'
              @input="val => handlePrice(val, scope.row)"
              :isEdit="(editType!='edit' || scope.row.year.indexOf('小计') >= 0) ? false : true"
            ></edit-grid-item>
          </template>
...
export default {
  ...
  computed: {
    showData() {
      const data = this.mpiShareholderCashFlowCalcInfo;
      if (data) {
    ...
    calcTotal(row){
      ...
      row.datas.forEach(element=>{
        if(element.year.indexOf('小计')<0){
          ...
          if(!curxj.price){
            curxj.price = element.price
          }
          else{
            curxj.price = curxj.price + element.price
          }
        }
      })
      let totalPrice = 0
